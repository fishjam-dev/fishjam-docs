...
Jellyfish Dashboard is a web application that allows you to manage your Jellyfish backend.
...
- **Server token** - a token necessary to authenticate the dashboard with the backend. For now, it's simply `development`,
but it is meant to authenticate the dashboard to the backend, so the good practice is to use a token that is hard to guess.
...
Logs are great for debugging purposes and provide insight into the various communications that occur between the dashboard and the backend.
