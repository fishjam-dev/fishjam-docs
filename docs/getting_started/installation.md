...
```bash
docker run -p 8080:8080/tcp -e JF_HOST=localhost:8080 -e JF_SERVER_API_TOKEN=token ghcr.io/jellyfish-dev/jellyfish:0.3.0
```
...
See for example the list of [Peers](./peers/webrtc.md), [HLS Component](./components/hls.md) and 
[SIP Component](./components/sip.md).
