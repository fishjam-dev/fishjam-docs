...
```bash
npx react-native@latest init JellyfishMobileExample && cd JellyfishMobileExample
```
...
```bash
npx react-native init JellyfishMobileExample --template react-native-template-typescript && cd JellyfishMobileExample
```
...
For physical devices, it would be easier to scan a QR code to place the peer token in the app. For a simulator/emulator, it is more convenient to copy the token to the clipboard and paste it in the simulator/emulator.
:::note
...
You can also add a fake peer that shares a video track in the dashboard (remember to check the `Attach metadata` checkbox):
